...
> Historical record. Sprint 3 and the MVP prototype were completed and owner
> accepted on 2026-08-15. Current planning authority is
...
**Active story:** None - roadmap archived

**Active story state:** All stories and all four sprint exit gates passed

**Implementation authorization:** None - repository transitioned to planning
pending for publication refinement

**Parent roadmap:**
[`DSP Recipe Tracker - MVP Roadmap`](../planning/MVP-ROADMAP.md)

**Previous roadmap:**
[`Sprint 2 - Native Integration and Tracker State`](UI_INTEGRATION_ROADMAP.md)
...
`0.1.318`. Source-ready and Package-inspected then passed again for clean
source revision `14bbe8e046e32333bd7cf68f35b8f22bc04dd47f` using build
number 318. The owner then accepted the completed Sprint 3 exit. Publication
approval and supported-release status remained separate and were not inferred.
...
The agent-delivered result was a deterministic, statically inspected, testable
DLL and package plus a concise owner procedure. Installed behavior was claimed
only after the owner performed and reported the human-only checks.
...
- Sprint 2 supplies transient ordered pins, native right-click input,
  ...
  orchestration.
- At Sprint 3 entry, installed gesture behavior, native-state coexistence,
  presentation suitability, panel composition, dragging, containment, live
  visibility, and cleanup remained unvalidated. They were grouped into the
  Sprint 3 owner checkpoint and later passed.
...
Visual suitability was deferred at this story gate and later passed in the
final owner procedure and refinement workshop.
...
3840-by-2160/Auto display case were deferred at this story gate and later
passed in the final owner procedure.
...
- **Accepted product-contract deviation:** The original full-cell green
  treatment for available recipes and red treatment for pinned recipes was
  replaced by non-raycasting green corner-bracket treatment on up to three
  pinned recipes. The live full-coverage implementation obscured artwork and
  became excessively bright on some icons; its cost was not justified by the
  utility of treating every available cell. Unpinned cells now remain neutral.
  Pin state stays immediately recognizable without additional cognitive load.
  A distinct available-recipe treatment was deferred beyond this prototype.
- **Accepted presentation compromise:** Native multiline text proved unreliable
  through several direct builds. Machine/facility names therefore use a full-
  width, single-line footer, increasing the fixed panel from 252 to 300 layout
  units so three complete rows remain contained.
- **Accepted presentation improvements:** The panel uses a flat dark
  translucent background without a redundant border. `TARGET` and
  `INGREDIENTS` headings, a restrained separator, aligned icons, centered and
  enlarged quantities, stronger sufficiency colors, and larger facility text
  provide explicit structure and legibility.
- **Accepted layout improvements:** The global control is placed 38 units above
  the native screenshot-button reference, and the initial panel position is
  derived from live layout height as true left-middle placement. Deterministic
  checks cover the 1080- and 1440-unit Auto layouts and undersized bounds.
- **Reliability correction:** Game-session shutdown releases transient pins
  and owned UI before a same- or different-save load initializes a new tracker
  session. The tracker does not persist state across game sessions.
...
build `0.1.318`. This completed S3-06 without inferring publication approval
or release candidacy. Source-ready and Package-inspected subsequently passed
for the committed workshop revision.
...
`14bbe8e046e32333bd7cf68f35b8f22bc04dd47f`

- Every included Sprint 3 story meets its definition of done and S3-01 through
  S3-06 are explicitly owner accepted. Sprint exit review remained separate.
...
Revalidated with `Validate-Sprint3-SourceReady.ps1` using build number 318. The
...
passed for the clean workshop commit. This gate does not establish live
behavior. The earlier build-310 result remains superseded historical evidence.
...
Revalidated with `Validate-Sprint3-PackageInspected.ps1` using build number 318.
Hosted and Local package construction and static inspection passed. The
retained files are `artifacts/package/0.1.318/DSPRecipeTracker.dll` and
`artifacts/package/0.1.318/DSPRecipeTracker-0.1.318.zip`; `build-info.json`
records `referenceMode` `Local`, diagnostic label
`0.1.318.14bbe8e046e3`, and the validated source revision. The archive contains
only the three root package assets and the intended plugin DLL. The earlier
build-310 result remains superseded historical evidence.
...
cleanup. The result applied only to the recorded test build and selected
conditions and did not by itself imply owner acceptance or release candidacy.

### Owner-reviewed

**Status:** Passed by explicit owner acceptance on 2026-08-15
...
The owner accepted the final workshop result and Sprint 3 exit after the
Source-ready, Package-inspected, and Behavioral-ready evidence was complete.
This established a working validated MVP prototype, not a publication-ready
or supported release.

## Explicitly unavailable states

Behavioral-ready was established only for the recorded test build and selected
conditions. Sprint 3 established Owner-reviewed acceptance, but did not
establish broader compatibility or display support, `Publication-ready`, or
